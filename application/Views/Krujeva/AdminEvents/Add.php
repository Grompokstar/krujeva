<div class="middle-container">
	...
	<?php $item = null;
		...
		if (isset($_GET['id'])) {
			$item = \Krujeva\Data\Events::get($_GET['id']);
		}
	?>
		...
		<div class="tr">
			<div class="td">
				<div class="admin-t-inner">
					<?php echo $item ? 'Редактирование мероприятия' : 'Добавление мероприятия' ?>
				</div>
			</div>
	...
	<div class="form-edit-block" name="form">
		...
		<div class="f-edit-admin-row">
			<label for="">Текст мероприятия</label>
			<textarea style="width: 1100px" class="suitup-textarea" name="text">
				<?php echo isset($item['text']) ? \Utils::eq($item['text']) : '' ?>
			</textarea>
		</div>

		<div class="login-btn btn-inline" on-click="saveClick">Сохранить мероприятие</div>
	</div>

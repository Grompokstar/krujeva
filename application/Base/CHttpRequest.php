	...
	class CHttpRequest {
		...
		public function getRestParams() {
			...
			if ($this->_restParams === null) {
				...
				if (function_exists('mb_parse_str'))
					mb_parse_str($this->getRawBody(), $result); else
		...
		public function getHostInfo($schema = '') {
			...
			if ($this->_hostInfo === null) {
				...
				if (isset($_SERVER['HTTP_HOST']))
					$this->_hostInfo = $http . '://' . $_SERVER['HTTP_HOST']; else {
		...
		public function getScriptUrl() {
			...
			if ($this->_scriptUrl === null) {
				...
				if (basename($_SERVER['SCRIPT_NAME']) === $scriptName)
					$this->_scriptUrl = $_SERVER['SCRIPT_NAME']; elseif (basename($_SERVER['PHP_SELF']) === $scriptName)
					$this->_scriptUrl = $_SERVER['PHP_SELF']; elseif (isset($_SERVER['ORIG_SCRIPT_NAME']) && basename($_SERVER['ORIG_SCRIPT_NAME']) === $scriptName)
					$this->_scriptUrl = $_SERVER['ORIG_SCRIPT_NAME']; elseif (($pos = strpos($_SERVER['PHP_SELF'], '/' . $scriptName)) !== false)
					$this->_scriptUrl = substr($_SERVER['SCRIPT_NAME'], 0, $pos) . '/' . $scriptName; elseif (isset($_SERVER['DOCUMENT_ROOT']) && strpos($_SERVER['SCRIPT_FILENAME'], $_SERVER['DOCUMENT_ROOT']) === 0)
					$this->_scriptUrl = str_replace('\\', '/', str_replace($_SERVER['DOCUMENT_ROOT'], '', $_SERVER['SCRIPT_FILENAME'])); else
		...
		public function getRequestUri() {
			...
			if ($this->_requestUri === null) {
				...
				$this->_requestUri = $_SERVER['HTTP_X_REWRITE_URL']; elseif (isset($_SERVER['REQUEST_URI'])) {
		...
		public function getRequestType() {
			...
			if (isset($_POST['_method']))
				return strtoupper($_POST['_method']); elseif (isset($_SERVER['HTTP_X_HTTP_METHOD_OVERRIDE']))
			...
			if ($a['params']['q'] === $b['params']['q'])
				if (!($a['type'] === '*' xor $b['type'] === '*'))
					...
					if (count($a['params']) === count($b['params']))
						return 0; else
						return count($a['params']) < count($b['params']) ? 1 : -1; // more specific takes precedence - whichever one doesn't have a * subType
					else
						return $a['subType'] === '*' ? 1 : -1; // more specific takes precedence - whichever one doesn't have a * type
				else
					return $a['type'] === '*' ? 1 : -1; else
		...
		public function sendFile($fileName, $content, $mimeType = null, $terminate = true) {
			...
			if (isset($_SERVER['HTTP_RANGE'])) {
				...
				if ($range[0] === '-')
					$contentStart = $fileSize - substr($range, 1); else {
					$range = explode('-', $range);

...
$connection = new \Data\Connection('host=10.145.113.23 dbname=shard1 user=glonass password=glonass');
...
function import($connection, $parent = null, $parentId = null) {
	$query = 'select * from emergency.organizations where department = 2';
	$args = [];
	...
	while ($row = $query->row()) {
		if ($organization = build($row)) {
			...
			$organization = \Police\Data\Organizations::insert($organization);

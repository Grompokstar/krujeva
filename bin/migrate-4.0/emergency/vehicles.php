...
select
		...
		vehicleprofiles.name as profilename,
	...
	left join emergency.vehicleprofiles on vehicleprofiles.id = vehicles.profileid
...
function getProfileId($name) {
	...
	if (!$profile = \Emergency\Data\VehicleProfiles::firstBy(['name' => $name])) {
		$profile = \Emergency\Data\VehicleProfiles::insert(['name' => $name]);
		echo "Vehicle profile {$profile['name']} created\n";
	}

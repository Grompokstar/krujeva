...
function getOrganizationId($alias) {
	if ($organization = \CallCenter\Data\Organizations::firstBy(['alias' => $alias])) {
...
function getProfileId($profileName) {
	if (!$profile = \CallCenter\Data\Profiles::firstBy(['name' => $profileName])) {
		$profile = \CallCenter\Data\Profiles::insert(['name' => $profileName]);
...
function run() {
	...
	foreach ($items as $item) {
		...
		$employee = \CallCenter\Data\Employees::insert($employee);

...
function getProfileId($profileName) {
	if (!$profile = \Medicine\Data\Profiles::firstBy(['name' => $profileName])) {
		$profile = \Medicine\Data\Profiles::insert(['name' => $profileName]);
...
function getPositionId($positionName) {
	if (!$position = \Medicine\Data\Positions::firstBy(['name' => $positionName])) {
		$position = \Medicine\Data\Positions::insert(['name' => $positionName]);
	}
...
function run() {
	...
	foreach ($items as $item) {
		...
		$employee = [
			...
			'positionid' => getPositionId(get($item, 'positionname')),
			'profileid' => getProfileId(get($item, 'profilename'))
		];

		$employee = \Medicine\Data\Employees::insert($employee);
